...
const amadeusAPI = `https://api.sandbox.amadeus.com/v1.2/airports/autocomplete?apikey=${config.amadeusAPIKey}`;
...
const _getCityIATACode = location => {
  return new Promise(async (resolve,reject) => {
    try {
      const locationAirportsCityCodes = await fetch(`${amadeusAPI}&term=${location}`).then(res => res.json());
      locationAirportsCityCodes.length ? resolve(locationAirportsCityCodes[0].value) : reject(`Unable to fetch IATA code for ${location}`);
    } catch(err){
      reject(`Unable to fetch IATA code for ${location}`)
    }
  })
...
generateSearchQuery('London','Glasgow','28 June 2018').then(res => console.log(res));
